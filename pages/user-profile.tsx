...
import { NavBarController } from '../packages/controllers/navbar-controller/NavBarController';
import { UserContext } from '../packages/services/context/user-context';
import { auth, onboardVendor } from '../packages/services/firebase/firebase.utils';
...
const UserProflePage: FC<UserProfileProps> = ({
	isSignUp,
	userName,
}: UserProfileProps) => {
				...
				{revealForm === null ? (
					<div className='flex flex-col lg:rounded-lg lg:bg-white lg:border-2 lg:h-80 lg:w-3/5 lg:mt-12'>
						<div className='flex flex-col lg:flex-row lg:justify-center lg:mt-14'>
							...
							<div className='mt-10 lg:mt-0 lg:mx-5'>
								<div className='flex flex-row border-b-2 pb-2 my-2 lg:w-72 w-full font-lg font-light'>
									<h3 className=''>Welcome </h3>
									<h3 className='capitalize ml-1'>{userData.fname}</h3>
								</div>
								<div className='font-light italic text-sm mb-4'>
								...
								<div className='flex flex-col align-center'>
									...
									<NavButton
										...
										bgColor='primary'
										css='mb-3'
										onClick={() => setRevealForm('password')}
									/>
									{userData.isVendor ? (
										<NavButton
											label='become a seller'
									...
									{userData.isVendor ? (
										<NavButton
											...
											onClick={() =>
												router.push(
													`https://goshippo.com/oauth/authorize?response_type=code&client_id=${process.env.NEXT_PUBLIC_SHIPPO_CLIENT_ID}&scope=*&state=${userData.uid}`,
												)
											}
										/>
									) : null}
					...
					<ChangeEmailFormController
						type={revealForm}
						onGoBack={() => setRevealForm(null)}
					/>
				)}
			</MainContentContainer>
		</div>
